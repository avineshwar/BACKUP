...
backup_volume_size=`echo $(($dir_to_backup * 2))`
backup_volume_size=`echo $(($dir_to_backup / 1024))`
backup_volume_size=`echo $(($dir_to_backup / 1024))`
backup_volume_size=`echo $(($dir_to_backup + 1))`

if [ -z "key" ]
then
		aws ec2 create-key-pair --key-name $USER-EC2-BACKUP-key --query 'KeyMaterial' --output text > $USER-EC2-BACKUP-key.pem
		chmod 400 $USER-EC2-BACKUP-key.pem
