...
then
	...
	then
		...
		then
			...
			ssh -o StrictHostkeyChecking=no -i $key.pem root@$public_ip "/sbin/newfs /dev/xbd2a && mkdir /mnt/mount_point && /sbin/mount /dev/xbd2a /mnt/mount_point" 2>/dev/null
			...
			sleep 45 # necessary to make the volume accessible post attachment.
			public_ip=$(aws ec2 describe-instances --output text | egrep $instance_id | cut -f16)
			fs_check=$(ssh -o StrictHostkeyChecking=no -i $key.pem root@$public_ip "file -s /dev/xbd2a" | cut -d ' ' -f2)
			if [ "$fs_check" = "data" ]
			then
				echo "The volume is raw. Moving forward to create a filesystem."
				ssh -o StrictHostkeyChecking=no -i $key.pem root@$public_ip "/sbin/newfs /dev/xbd2a && mkdir /mnt/mount_point && /sbin/mount /dev/xbd2a /mnt/mount_point"
		
			else
				echo "The volume is not raw. Hoping it to be a supported one."
				ssh -o StrictHostkeyChecking=no -i $key.pem root@$public_ip "mkdir /mnt/mount_point && /sbin/mount /dev/xbd2a /mnt/mount_point"
				if [ $(echo $?) != 0 ]
				then
